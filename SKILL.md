...
- 不允许在业务逻辑、模板和样式中散落硬编码值；接口路径、状态值、颜色、尺寸、延迟时间、分页数量、提示文本、权限标识等稳定值必须抽成常量、枚举、配置或主题变量。
- 定义常量和枚举时，必须通过命名或注释说明其业务含义、取值来源和使用场景；如果值来自后端协议、产品约定或第三方限制，必须写清来源。
...
- 常量、枚举、配置项的注释必须说明“这个值代表什么”，而不是重复变量名。
...
- 新增普通 `.ts` 文件顶部必须添加文件头注释，说明功能、职责、范围、文件名、日期和作者；`.d.ts`、自动生成文件不套用普通 `.ts` 文件头模板。
- 工具文件顶部必须写清职责。
- 业务模块入口文件应说明模块边界。
...
/**
 * 功能描述
 * 职责：说明本文件负责的核心能力或业务边界。
 * 范围：说明本文件适用范围、调用边界或不应承载的职责。
 * @file  ${FILE_NAME}
 * @date ${DATE}
 * @author xxx
 */
...
- 开发多个页面时，必须保持页面结构、间距、字号、颜色、表格/表单/按钮状态和空态风格统一，不能每个页面各写一套视觉规则。
- 页面和组件样式必须优先使用系统主题变量和公共样式能力；如现有颜色、间距、层级变量不足，可在公共颜色变量或公共变量模块中新增语义化变量，再由页面复用。
...
- 在业务逻辑、模板或样式中散落硬编码值，未抽成常量、枚举、配置或主题变量
- 定义常量或枚举但没有说明业务含义、取值来源或使用场景
- 新增路由页面但没有同步路由配置
- 开发公用组件但没有在 `src/views/Demo` 添加或更新使用示例
- 新增 `.vue` 或普通 `.ts` 文件但没有按模板添加文件头注释
- 多个页面视觉风格不统一，或绕过主题变量重复硬编码颜色、字号、间距
- 无节制把低频组件注册为全局组件
...
- 返回值和副作用是否说明清楚。
- 是否存在本可避免的 `any`。
- 是否存在可抽离的硬编码值，尤其是状态值、颜色、尺寸、延迟时间、分页数量、提示文本、权限标识和接口路径。
- 新增常量或枚举是否说明了业务含义、取值来源和使用场景。
...
- 多页面开发时，页面视觉、布局、间距、交互状态是否保持统一。
- 是否优先使用系统主题变量；新增颜色或样式变量是否沉淀到公共变量模块并使用语义化命名。
- 是否残留调试代码和无意义日志。
